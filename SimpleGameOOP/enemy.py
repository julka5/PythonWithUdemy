...
class Troll(Enemy):
    def __init__(self, name):
        # Enemy.__init__(self, name=name, hit_points=23, lives=1) - used in Python 2
        # super(Troll, self).__init__( name=name, hit_points=23, lives=1) - same result as one below, needed when subclass?
...
class Vampire(Enemy):
    ...
    def dogdes(self):
        ...
        if random.randint(1, 3) == 3:
            print("**** {0._name} dogdes ****".format(self))
            return True
        else:
            return False
...
class VampireKing(Vampire):
    ...
    def __init__(self, name):
        # super(Vampire, self).__init__(name=name, hit_points=140, lives=3)
        super().__init__(name)
